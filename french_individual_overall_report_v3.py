...
french_speaker_list = ['014'] # Right now only 014 is a french speaker, but we can add more later if needed


for participant_id in list_of_IDs:

    # Initialize french_speaker variable
    french_speaker = False

    print(f'PROCESSING PARTICIPANT DD_{participant_id}')

    # Check if participant is a french speaker
    if participant_id in french_speaker_list:
        french_speaker = True
        print(f'Participant DD_{participant_id} is a french speaker.')
